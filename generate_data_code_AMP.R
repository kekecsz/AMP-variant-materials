...
erotic_trial_size_per_participant = 9
...
for(i in 1:num_sim){
  ...
  variable_names = c("timestamp", "participant_ID", "experimenter_ID_code", "experimenter_ASGS_total_score", 
                     "laboratory_ID_code", "sitePI_ASGS_total_score", "session_type", "consent_screen_answer", 
                     "refused_to_answer_sexual_orientation_question", "age", "sex", "final_consent", "ESP_Q_item_1", 
                     "ESP_Q_item_2", "ESP_Q_item_3", "SS_Q_item_1", "SS_Q_item_2", "trial_number", "guessed_side", 
                     "target_side", "reward_type", "sides_match", 
                     "trial_type", "available_trial_type", "in_lab")
  subj_data = as.data.frame(matrix(NA, nrow = 1, ncol = length(variable_names)))
  names(subj_data) = variable_names
  ...
  # generate reward type
  
  reward_type_start = rep(NA, 5)
  reward_type_true = sample(rep(c("erotic", "neutral"), each = erotic_trial_size_per_participant), replace = F)
  reward_type_sham = sample(rep(c("erotic", "neutral"), each = erotic_trial_size_per_participant), replace = F)
  reward_type_true_sham = c(reward_type_true, reward_type_sham)
  
  # generate trial type
  
  trial_type_start = rep(NA, 5)
  trial_type = rep(c("t", "sh"), each = erotic_trial_size_per_participant*2)
  trial_type_sampling_index = sample(1:(erotic_trial_size_per_participant*2*2)) 
  
  # randomize trial type order while maintaining reward type balance
  
  reward_type_true_sham_mixed = reward_type_true_sham[trial_type_sampling_index]
  trial_type_mixed = trial_type[trial_type_sampling_index]
  
  reward_type = c(reward_type_start, reward_type_true_sham_mixed)
  trial_type = c(trial_type_start, trial_type_mixed)
  
  available_trial_type = 4
  
  in_lab = 0
  
  
  for(j in 1:((erotic_trial_size_per_participant*2*2)+5)){
    ...
    subj_data[j, "trial_type"] = if(j == 1 | j == 2 | j == 3 | j == 4 | j == 5){NA} else {trial_type[j]}
    subj_data[j, "available_trial_type"] = available_trial_type
    subj_data[j, "in_lab"] = in_lab
...
### testing erotic/neutral reward balance in sham and true trials

nrow(na.omit(raw_data[raw_data[,"participant_ID"] == "part_1" & raw_data[,"reward_type"] == "erotic" & raw_data[,"trial_type"] == "t",]))
nrow(na.omit(raw_data[raw_data[,"participant_ID"] == "part_1" & raw_data[,"reward_type"] == "erotic" & raw_data[,"trial_type"] == "sh",]))
